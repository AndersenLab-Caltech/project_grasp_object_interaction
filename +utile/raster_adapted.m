function [fig,ax] = raster_adapted(varargin)
%Sarah: Took spencers functions and adapted it to plot bars that represent phase
%changes 
...
%   the legend).
%
%   [...] = RASTER(...,'phase_time_changes', phase_time_changes)
%   Plots vertical lines at the provided timestamps.

%   [...] = RASTER(...,'AudioData', AudioData)
%   Plot the audio sound of the trial if present
%
%   [...] = RASTER(...,'INVISIBLE')
...
%   [...] = RASTER(...,'saveFigureName',name)
%   Provide a name to save a figure. The figure is automatically saved
%   under the datafile name if provided. 
%
...
% process inputs
[varargin,fs] = Utilities.argkeyval('fs',varargin,nan);
[varargin,flagShowPSTH] = Utilities.argkeyval('psth',varargin,true);
[varargin,flagShowRaster] = Utilities.argkeyval('raster',varargin,true);
assert(flagShowPSTH||flagShowRaster,'Must enable at least one of the PSTH or raster subplots');
[varargin,rasterXLabel,~,found_rasterXLabel] = Utilities.argkeyval('xlabel',varargin,nan);
[varargin,rasterXLim,~,found_rasterXLim] = Utilities.argkeyval('xlim',varargin,nan);
[varargin,sorting] = Utilities.argkeyval('sort',varargin,'none'); % 'none','fr_asc','fr_desc'
[varargin,groups,~,found_groups] = Utilities.argkeyval('groups',varargin,nan);
[varargin,glabels] = Utilities.argkeyval('grouplabels',varargin,nan);
[varargin, phase_start_time] = Utilities.argkeyval('phase_time_changes',varargin,[]);
[varargin,flagVisible] = Utilities.argflag('invisible',varargin,true);
[varargin,titlestr] = Utilities.argkeyval('title',varargin,'');
[varargin,save_figure_name] = Utilities.argkeyval('saveFigureName',varargin,nan);
[varargin,Audio] = Utilities.argkeyval('AudioData',varargin,[]);
[varargin,PhaseNames] = Utilities.argkeyval('PhaseNames',varargin,cell(1,length(phase_start_time)));

flagShowTitle = ~isempty(titlestr);
[varargin,markersize] = Utilities.argkeyval('markersize',varargin,3);
[varargin,fig] = Utilities.argisa('matlab.ui.Figure',varargin,[]);
[varargin,debug,found] = Utilities.argisa('Debug.Debugger',varargin,nan);

groups_orig_labels = cell2mat(groups'); 
...
if iscell(spikes)
    ...
    if ~all(cellfun(@issparse,spikes))
        ...
        if isnan(t)
            %num_timestamps = max(cellfun(@max,spikes), 'UniformOutput', false); warning('maybe change back without uniform output')
            nonempty_idx =  find(~cellfun('isempty', spikes)); %take care of emppty cells
            nonempty_spikes = spikes(nonempty_idx);
            num_timestamps = max(cellfun(@max,nonempty_spikes)); 
        ...
        spikes = Utilities.ascell(spikes);
...
% rearrange by group
groups = Utilities.ascell(groups);
...
for gg=1:numGroups
    ...
    [r{gg},tau{gg},e{gg}] = proc.basic.psth(spikes(:,groups{gg}),t,debug,'alpha',0.05,'kernel_width',diff(t([1 kernel_sd+1])),'trialave'); 
    %Have to  use trialaverage otherwise won't work, because r will be per individual trial!!!
    e{gg}(1,:) = r{gg} - e{gg}(1,:)';
    e{gg}(2,:) = e{gg}(2,:) - r{gg}';
...
if flagShowPSTH && flagShowRaster
    if flagShowTitle
        ...
        %yticklabels(groups_orig_labels) %put the number of trials as y label 
...
if size(colors,1)<numGroups
    colors = utile.distinguishable_colors(numGroups);
end

%give personalized colors to figure if labels are present. 
if iscell(glabels)    
   %colors = cell2mat(cellfun(@(x) preproc.image2class_simple(x), glabels));
   colors = cell2mat(cellfun(@(x) utile.get_color_rgb_codes({x}), glabels));
...
if flagShowRaster
    ...
    if iscell(glabels) && ~flagShowPSTH
        ...
        %legend(hAxesRaster,cat(1,handles{:}),glabels,'Interpreter','none','AutoUpdate','off','Location', 'southoutside');
        %legend(hAxesRaster,cat(1,handles{:}),glabels,'Interpreter','none','AutoUpdate','off','Location', 'west');
...
if flagShowPSTH
    ...
    [hl,hp] = Utilities.boundedline(xx,yy,zz,'alpha','cmap',colors(1:numGroups,:),'transparency',0.5,hAxesPSTH);
    set(hl,'LineWidth',2);
    
    % plot audio signal if provided
    if ~isempty(Audio)
        yyaxis right
        try
            a= plot((1:length(Audio))/fs, Audio, 'k', 'LineWidth', 2); 
        catch
            error(['Must provide input of type "double" for Audio (single Audio file per trial). Current type is: ' class(Audio)])
        end
    ...
    if iscell(glabels)
        ...
        if ~isempty(Audio)
            ...
            glabels{end + 1} = 'Audio Envelope';
        end
        legend(hAxesPSTH,handles,glabels,'Interpreter','none','AutoUpdate','off', 'location', 'northwest');
    end
...
%Plot vertical lines representing changes in phase
hold on
if ~isnan(phase_start_time)   
    
    if ~isnan(fs)
        phase_start_time = phase_start_time/fs; 
    end
    
    
     for i = 1:length(phase_start_time) 
         yL = get(gca,'YLim');
         phase_change = phase_start_time(i);
         xline(phase_change, 'k--', PhaseNames{i},'Linewidth', 1);
         
        % l(i) = line([phase_change, phase_change],yL,'Color','k','LineStyle','--','Linewidth', 1);
     end 

end 

%If figure name is provided, save figure
if ischar(save_figure_name)
    saveas(fig, save_figure_name); 
    disp(['Figure  '  titlestr ' saved']);    
end 