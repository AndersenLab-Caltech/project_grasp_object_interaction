function [] = plot_tuning_curves(groups,time_phase_labels,Feature_number,unit_number_c, Figure_labels, varargin)
    ...
    f =   figure('units','normalized','outerposition',[0 0 1 0.7]);
    FontSize = 20; 
    ...
    for con = 1:condition
        ...
        if con == condition
            if  isempty(Audio_file)
                ...
                legend([l(1),a, h{:}],[{' Phase Start','Mean Audio file' },names{:}], 'Location', 'best', 'FontSize', FontSize);

            end 
            %plot vertical text (names) of phases with small offset
            hold on 
            ht = text(time_points+0.3, repmat(ylim_val(2)-ylim_val(2)/4, [1 length(time_points)]), {'ITI', 'Image Cue', 'Delay' 'Action'},'FontSize',FontSize+5);
            set(ht,'Rotation',90)
        end 
