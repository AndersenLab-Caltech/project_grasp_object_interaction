function [TunedCombinedChannels, TunedChannelsPerPhase, TunedChannelsPerBin, sumPhase, sumBin,NbrTunedChannelsPerGrasp,NumberOfUnitsTunedToSeveralGrasps, TunedChannelsPerPhasePerGrasp,NbrTunedChannelsPerGraspBin,p_per_phase, p_per_phaseOrig] = getRegressionTunedChannels_uneven_trial_number(Data,Labels, TimePhaseLabels,varargin)
...
Condition_names = [{'ITI'}];
Condition = repmat(Condition_names, [max(fixed_trial_number),1]);

%ConLabels = arrayfun(@(x) preproc.image2class_simple(x), Labels, 'UniformOutput', false);
...
for PhaseNbr = 1:numPhases
  ...
  for ChannelNbr = 1:size(DataPerPhase,2)
      ...
      DataPerTrialOrdered = DataPerTrial; 
      ...
      %same ITI EVERYWHERE
      ITI_Data = ones(max(fixed_trial_number),1)*mean(mean(ITI_DataAll(:,ChannelNbr)));
    ...
    for BinPhaseNbr = 1:numBins
        ...
        for ChannelNbr = 1:numChannels
         ...
         % ITI_Data = ones(fixed_trial_number,1)*mean(mean(ITI_DataAll(:,ChannelNbr)));
          %remove order! otherwise it's wrong in this code
          ...
          if multipleComparePhase
              [~,~,p_val_bin] = Utilities.MultipleComparisonsCorrection(p_val_bin,'method', 'fdr');
          end 
          p_per_bin(BinPhaseNbr,:,ChannelNbr) = p_val_bin; %Grasps in conrrect order
