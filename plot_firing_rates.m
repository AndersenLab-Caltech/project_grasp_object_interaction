...
%% analyzing fr for aperture size/object type (can sep by grasp type)
if strcmp(taskName, 'GraspObject_Varied_Size')
    for n_brain = 1%:length(brainAreas) % 1:5 for AN, 1:3 for FG, [1, 3:6] for GB
        ...
        for n_channel = 39%1:numChannels
            ...
            %for n_grasp = 1:numel(uniqueGraspTypes)
                ...
                color_info = {[.4688 .3672 .9375],[.6016 .5234 .9531],[.7305 .6797 .9648]}; % different shades of purple for sizes
...
for n_brain = 1%:length(brainAreas) % 1:5 for AN, 1:3 for FG, [1, 3:6] for GB
    ...
    for n_channel = 3%1:numChannels
        ...
        xlim([30 134]) % 174 (# timebins) + 5 (buffer)
        xticks([]); % corresponds to phases and end of trial
...
for n_brain = 6%:length(brainAreas) % 1:5 for AN, 1:3 for FG, [1, 3:6] for GB
    
    frData = Data.frPerChannel{n_brain};
    numChannels = size(frData, 1);
    
    for n_channel = 1%:numChannels
        figure('units', 'normalized', 'outerposition', [0 0 0.25 0.27])
        %sgtitle([brainAreas{n_brain} ' - Channel ' num2str(n_channel)], 'FontWeight', 'bold');
        hold on;
...
%% GS, Size Parameters 
%phase_bins = 42:82; % cue
phase_bins = 94:174; % action
alpha = 0.05;
